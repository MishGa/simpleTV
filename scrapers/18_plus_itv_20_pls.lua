-- скрапер TVS для загрузки видео плейлиста "18+" https://itv.rt.ru (9/12/19)
	module('18_plus_itv_20_pls', package.seeall)
	local my_src_name = '18+'
	function GetSettings()
		local scrap_settings = {name = my_src_name, sortname = '', scraper = '', m3u = 'out_' .. my_src_name .. '.m3u', logo = '..\\Channel\\logo\\Icons\\18_plus_rt.png', TypeSource = 1, TypeCoding = 1, DeleteM3U = 0, RefreshButton = 0, AutoBuild = 0, show_progress = 1, AutoBuildDay = {0, 0, 0, 0, 0, 0, 0}, LastStart = 0, TVS = {add = 0, FilterCH = 0, FilterGR = 0, GetGroup = 0, LogoTVG = 0}, STV = {add = 1, ExtFilter = 0, FilterCH = 0, FilterGR = 0, GetGroup = 1, HDGroup = 0, AutoSearch = 0, AutoNumber = 0, NumberM3U = 0, GetSettings = 1, NotDeleteCH = 0, TypeSkip = 1, TypeFind = 1, TypeMedia = 1}}
	...
	local function LoadFromSite(p)
		...
		m_simpleTV.OSD.ShowMessageT({text = '«18+» загрузка ...', color = 0xff9bffff, showTime = 1000 * 200, id = 'channelName'})
		...
		local j = 1
			while true do
				...
				if tab.movie_list[j].assets
					...
					and tab.movie_list[j].isErotic == '1'
				then
					t[i] = {}
