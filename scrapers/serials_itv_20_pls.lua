-- скрапер TVS для загрузки видео плейлиста "Сериалы" https://itv.rt.ru (9/12/19)
	module('serials_itv_20_pls', package.seeall)
	local my_src_name = 'Сериалы'
	function GetSettings()
		local scrap_settings = {name = my_src_name, sortname = '', scraper = '', m3u = 'out_' .. my_src_name .. '.m3u', logo = '..\\Channel\\logo\\Icons\\serials_rt.png', TypeSource = 1, TypeCoding = 1, DeleteM3U = 0, RefreshButton = 0, AutoBuild = 0, show_progress = 1, AutoBuildDay = {0, 0, 0, 0, 0, 0, 0}, LastStart = 0, TVS = {add = 0, FilterCH = 0, FilterGR = 0, GetGroup = 0, LogoTVG = 0}, STV = {add = 1, ExtFilter = 0, FilterCH = 0, FilterGR = 0, GetGroup = 1, HDGroup = 0, AutoSearch = 0, AutoNumber = 0, NumberM3U = 0, GetSettings = 1, NotDeleteCH = 0, TypeSkip = 1, TypeFind = 1, TypeMedia = 2}}
	...
	local function LoadFromSite(p)
		...
		m_simpleTV.OSD.ShowMessageT({text = '«Сериалы» загрузка ...', color = 0xff9bffff, showTime = 1000 * 200, id = 'channelName'})
		...
		local j = 1
			while true do
				...
				if tab.movie_list[j].assets
					...
					and tab.movie_list[j].assets.ott_content_asset.ifn ~= ''
					and tab.movie_list[j].isErotic
					and tab.movie_list[j].isErotic == '0'
					and tab.movie_list[j].isChild
					and tab.movie_list[j].isChild == '0'
					and tab.movie_list[j].genres
					and tab.movie_list[j].genres[1]
					and tab.movie_list[j].genres[1] ~= '1744096'
					and tab.movie_list[j].genres[1] ~= '73409267'
					and tab.movie_list[j].genres[1] ~= '16519276'
					and tab.movie_list[j].genres[1] ~= '28808574'
				then
					t[i] = {}
					...
					t[i].actors = tab.movie_list[j].actors
					t[i].director = tab.movie_list[j].director
					i = i + 1
				end
				j = j + 1
			end
			if i == 1 then return end
		local reting, year, country, video_title, director, actors, desc
			for i = 1, #t do
				...
				if t[i].country then
					country = ' | ' .. t[i].country:gsub(',', '%%2C ')
				end
				video_title = (country or '') .. (year or '')
				video_title = video_title:gsub('|%s*|', '|'):gsub('%s+', ' '):gsub('^[%s|]*(.-)%s*$', '%1')
				t[i].video_title = video_title
				if t[i].director and t[i].director ~= '' then
					director = '✽ Режисер: ' .. t[i].director
				end
				if t[i].actors and t[i].actors ~= '' then
					actors = ' ✽ Актеры:  ' .. t[i].actors
				end
				if director or actors then
					desc = (director or '') .. (actors or '')
					desc = desc:gsub(',', '%%2C'):gsub('%c', '')
					t[i].video_desc = desc
				end
			end
		local hash, t0 = {}, {}
	...
	function GetList(UpdateID, m3u_file)
		...
		local w = decode64('aHR0cHM6Ly9mZS5zdmMuaXB0di5ydC5ydS9DYWNoZUNsaWVudEpzb24vanNvbi9Wb2RQYWNrYWdlL2xpc3RfbW92aWVzP2xvY2F0aW9uSWQ9NzAwMDAxJmZyb209MCZ0bz05OTk5OTk5OSZnZW5yZUlkPTIzNzM4MDQxJnBhY2thZ2VJZD0xMDAwLDIxNzEzOTQy')
