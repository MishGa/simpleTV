...
----------------------------------------------------------------------------------
	module('sport_pls', package.seeall)
	local my_src_name = 'Спорт'
	...
	local function LoadFromSite()
		...
		local rc, answer = m_simpleTV.Http.Request(session, {url = decode64('aHR0cDovL21hcGkubnR2cGx1cy50di92MS90di9jaGFubmVscw==')})
		...
		local tab = json.decode(answer)
			...
			while true do
				...
				t[i] = {}
				t[i].name = tab[i].name:gsub('%(', ''):gsub('%)', '')
				t[i].address = 'http://sport.ntv/' .. tab[i].streamServerId
				i = i + 1
			end
	...
	function GetSettings()
	 return {name = my_src_name, sortname = '', scraper = '', m3u = 'out_' .. my_src_name .. '.m3u', logo = '..\\Channel\\logo\\Icons\\sport.png', TypeSource = 1, TypeCoding = 1, DeleteM3U = 1, RefreshButton = 1, AutoBuild = 0, AutoBuildDay = {0, 0, 0, 0, 0, 0, 0}, LastStart = 0, TVS = {add = 1, FilterCH = 1, FilterGR = 1, GetGroup = 1, LogoTVG = 1}, STV = {add = 0, ExtFilter = 1, FilterCH = 1, FilterGR = 1, GetGroup = 1, HDGroup = 1, AutoSearch = 1, AutoNumber = 1, NumberM3U = 0, GetSettings = 1, NotDeleteCH = 0, TypeSkip = 1, TypeFind = 1, TypeMedia = 0}}
	end
	function GetVersion()
	 return 2, 'UTF-8'
	end
