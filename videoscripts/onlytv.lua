-- видеоскрипт для сайта http://only-tv.org (26/8/20)
...
-- ##
	...
	local inAdr = m_simpleTV.Control.CurrentAddress
		if not m_simpleTV.Control.CurrentAddress:match('^https?://only%-tv%.') then return end
	local inAdr = m_simpleTV.Control.CurrentAddress
	...
	m_simpleTV.Http.SetTimeout(sessionNoPrx, 8000)
