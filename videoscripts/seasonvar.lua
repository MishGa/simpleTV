-- видеоскрипт для сайта http://seasonvar.ru (18/4/20)
-- логин, пароль установить в 'Password Manager', для id - seasonvar
-- необходимы скрипты: pladform, youtube, russiatv, ovvatv, megogo
-- открывает подобные ссылки:
-- http://seasonvar.ru/serial-18656-Lyudi-3-season.html
------------------------------------------------------------------------------------------
local noad = 0 -- пропускать рекламу в начале, сек.
------------------------------------------------------------------------------------------
local proxy = 'http://proxy-nossl.antizapret.prostovpn.org:29976' -- прокси
-- '' - нет
...
------------------------------------------------------------------------------------------
	...
	if not inAdr:match('^$seasonvar') and not inAdr:match('&kinopoisk') then
		if m_simpleTV.Control.MainMode == 0 then
			m_simpleTV.Interface.SetBackground({BackColor = 0, BackColorEnd = 255, PictFileName = 'http://seasonvar.ru/tpl/asset/img/footer.logo.png', TypeBackColor = 0, UseLogo = 1, Once = 1})
		end
	...
	local function GetAddressFromPlaylist(data)
		...
		if #a > 1
			and m_simpleTV.Common.GetVersion() > 830
			and m_simpleTV.User.Seasonvar.Cookies ~= ''
		then
	...
	function Seasonvar_OnMultiAddressCancel(Object)
		if m_simpleTV.User.Seasonvar.DelayedAddress ~= nil then
			...
			if state == 0 then
				m_simpleTV.Control.SetNewAddress(m_simpleTV.User.Seasonvar.DelayedAddress)
			end
			m_simpleTV.User.Seasonvar.DelayedAddress = nil
		end
	end
	...
	retAdr = retAdr:gsub('^$seasonvar', '') .. '$OPT:start-time=' .. noad
	m_simpleTV.Control.CurrentAddress = retAdr
