-- видеоскрипт для https://cloud.mail.ru (18/4/20)
-- открывает подобные ссылки
-- https://cloud.mail.ru/public/GuR9/CpdDRwxu1
-- https://cloud.mail.ru/public/LHJZ/DGrDhjz72
-- https://cloud.mail.ru/public/5zQt/To1tr3NKX/
-- https://cloud.mail.ru/public/4VZcMg86gm1s/%D0%90%D1%83%D0%B4%D0%B8%D0%BE%D0%BA%D0%BD%D0%B8%D0%B3%D0%B8
	...
	function SavePlst_Mailru()
		if m_simpleTV.User.Mailru.Plst and m_simpleTV.User.Mailru.plstHeader then
			...
			if fhandle then
				...
				fhandle:close()
				m_simpleTV.OSD.ShowMessageT({text = 'плейлист сохранен', color = ARGB(255, 155, 155, 255), showTime = 1000 * 5, id = 'channelName'})
			else
				m_simpleTV.OSD.ShowMessageT({text = 'ошибка сохраненения', color = ARGB(255, 155, 155, 255), showTime = 1000 * 5, id = 'channelName'})
			end
		end
	end
	local function getmailkey()
		...
		local session2 = m_simpleTV.Http.New('Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML,like Gecko) Chrome/68.0.2785.143 Safari/537.36')
			if not session2 then return end
		m_simpleTV.Http.SetTimeout(session2, 8000)
		local rc, answer = m_simpleTV.Http.Request(session2, {url = 'https://cloud.mail.ru/api/v2/tokens/download', method = 'post', headers = 'Content-Type: application/x-www-form-urlencoded; charset=UTF-8', body = 'api=2&build=release-33.201602101549'})
	...
	local rc, answer = m_simpleTV.Http.Request(session, {url = inAdr})
		if rc ~= 200 then
			...
			m_simpleTV.OSD.ShowMessageT({text = 'cloud_mailru ошибка[1]-' .. rc, color = ARGB(255, 255, 0, 0), showTime = 1000 * 5, id = 'channelName'})
		 return
		end
	m_simpleTV.Http.Close(session)
	local extOpt = '$OPT:NO-STIMESHIFT'
	local host = answer:match('weblink_get".-url":%s*"(.-)"') or 'https://datacloudmail.ru/weblink/get'
	local tmp = answer:match('"folders".-("tree".-);</script>')
	...
	local retAdr, name, kind, link, count
		...
		if not retAdr then
			...
			m_simpleTV.OSD.ShowMessageT({text = 'Медиа файлов не найдено', color = ARGB(255, 255, 0, 0), showTime = 1000 * 5, id = 'channelName'})
			m_simpleTV.Control.CurrentAddress = 'error'
		 return
		end
	if n > 2 then
