-- видеоскрипт для сайта http://premier.one (16/5/20)
-- ## Авторизация ##
...
-- https://premier.one/show/10085
-- https://premier.one/show/9101/season/1/video/28893dfac205ca129b3a396a225187ed
			...
			local function GetNetscapeFileFormat()
				...
				local name, val
					for line in fhandle:lines() do
						_, _, _, name, val = line:match('^[%#%a_%.]*premier%.one%s+(%a+)%s+/%s+(%a+)%s+(%d+)%s+(.-)%s+(.-)$')
	...
	local headers = 'Cookie: ' .. m_simpleTV.User.premierOne.cookies
