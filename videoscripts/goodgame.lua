-- видеоскрипт для сайта https://goodgame.ru (26/3/19)
-- необходимы скрипты: twitch, youtube
...
-- https://goodgame.ru/clip/506085/
	...
	local userAgent = 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3945.122 Safari/537.36'
	...
	local name, adr
		for w in answer1:gmatch('EXT%-X%-STREAM%-INF(.-%.m3u8)') do
			...
			if adr and name then
				...
				t[i].Name = name .. 'p'
				t[i].Address = 'https://hls.goodgame.ru' .. adr .. extOpt
				i = i + 1
			end
		end
		if i == 1 then
			...
			local smil = channelTopInfo:match('"source":"([^"]+)')
				if smil then
					...
					smil = smil .. extOpt
					m_simpleTV.Control.CurrentAddress = smil
					m_simpleTV.Control.CurrentTitle_UTF8 = title
					m_simpleTV.OSD.ShowMessageT({text = title, showTime = 1000 * 5, color = 0xff9bffff, id = 'channelName'})
				 return
				end
