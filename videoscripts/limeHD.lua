-- видеоскрипт для плейлиста "LimeHD" https://new.info-link.ru (23/7/20)
-- необходим скрапер TVS: LimeHD_pls
			...
			if tonumber(id) == tonumber(m_simpleTV.User.infolink.plst[i].ServerId) then
				...
				if m_simpleTV.User.infolink.plst[i].with_archive == true then
					...
					then
						...
						m_simpleTV.User.infolink.url_archive = url_archive .. 'index.m3u8' .. extopt
					end
				end
			 break
			end
